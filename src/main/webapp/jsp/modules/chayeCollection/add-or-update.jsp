            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑茶叶收藏</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 chayeCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="chayeCollectionTypesSelect" name="chayeCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("chayeCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    // 查询列表
        ...
        function chayeCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chaye_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    chayeCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationChayecollectiontypesSelect(){
            var chayeCollectionTypesSelect = document.getElementById('chayeCollectionTypesSelect');
            if(chayeCollectionTypesSelect != null && chayeCollectionTypesOptions != null  && chayeCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < chayeCollectionTypesOptions.length; i++) {
                    chayeCollectionTypesSelect.add(new Option(chayeCollectionTypesOptions[i].indexName,chayeCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(chayeCollectionTypesSelect != null && chayeCollectionTypesOptions != null  && chayeCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < chayeCollectionTypesOptions.length; i++) {
                if (chayeCollectionTypesOptions[i].codeIndex == ruleForm.chayeCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        chayeCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                chayeId: "required",
                yonghuId: "required",
                chayeCollectionTypes: "required",
            },
            messages: {
                chayeId: "茶叶不能为空",
                yonghuId: "用户不能为空",
                chayeCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchayeCollection = window.sessionStorage.getItem("addchayeCollection");
        if (addchayeCollection != null && addchayeCollection != "" && addchayeCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("chayeCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationChayecollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
