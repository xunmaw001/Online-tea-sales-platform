...
<body>
    <div class="modal fade" id="plusOrReduceChayeKucunNumberModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="plusOrReduceChayeKucunNumberTitle">增加或减少库存</h5>
                ...
                <div class="modal-body">
                    数量:<input type="number" id="plusOrReduceChayeKucunNumber" style="width: 300px;" class="form-control form-control-sm"
                    ...
                    <input type="hidden" id="chayeId">
                    <input type="hidden" id="hideflag">
                    <input type="hidden" id="thisChayeKucunNumber">
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" onclick="submitPlusOrReduceChayeKucunNumber()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
...
<!--/Pre Loader -->
                ...
                <div class="row breadcrumb-bar">
                    ...
                    <div class="col-md-6">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">茶叶列表</li>
                        </ol>
                    </div>
                </div>
            </div>
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //茶叶库存
            var chayeKucunNumberCell = document.createElement('td');
            chayeKucunNumberCell.innerHTML = item.chayeKucunNumber;
            row.appendChild(chayeKucunNumberCell);


            //购买获得积分
            var chayePriceCell = document.createElement('td');
            chayePriceCell.innerHTML = item.chayePrice;
            row.appendChild(chayePriceCell);


            //茶叶原价
            var chayeOldMoneyCell = document.createElement('td');
            chayeOldMoneyCell.innerHTML = item.chayeOldMoney;
            row.appendChild(chayeOldMoneyCell);


            //现价/积分
            var chayeNewMoneyCell = document.createElement('td');
            chayeNewMoneyCell.innerHTML = item.chayeNewMoney;
            row.appendChild(chayeNewMoneyCell);


            //点击次数
            var chayeClicknumCell = document.createElement('td');
            chayeClicknumCell.innerHTML = item.chayeClicknum;
            row.appendChild(chayeClicknumCell);
            ...
            var plusAttr = "plusChayeKucunNumber(" + item.id+','+item.chayeKucunNumber+")";
            ...
            var reduceAttr = "reduceChayeKucunNumber(" + item.id+','+item.chayeKucunNumber+')';
        ...
        function remove(id) {
            ...
            if (mymessage == true) {
                ...
                httpJson("chaye/delete", "POST", paramArray, (res) => {
                    if(res.code == 0){
                        getDataList();
                        alert('删除成功');
                    }
        ...
        //添加
        function add() {
            window.sessionStorage.setItem("addchaye", "addchaye");
            window.location.href = "add-or-update.jsp"
        }
        ...
        function shangxia(id,shangxiaTypes) {
            ...
            httpJson("chaye/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList()
                }else{
    ...
    //查询当前表搜索条件所有列表
        ...
        //增加库存
        function plusChayeKucunNumber(thisId,chayeKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceChayeKucunNumberTitle").val("增加库存");
            $("#thisChayeKucunNumber").val(chayeKucunNumber);
            $("#hideflag").val("+");
            $("#chayeId").val(thisId);
            id=thisId;
            $('#plusOrReduceChayeKucunNumberModal').modal('show');
        }

        //减少库存
        function reduceChayeKucunNumber(thisId,chayeKucunNumber) {
            if(thisId == null || thisId=="" || thisId=="null"){
                alert("未知错误,请联系管理员处理");
                return;
            }
            $("#plusOrReduceChayeKucunNumberTitle").val("减少库存");
            $("#thisChayeKucunNumber").val(chayeKucunNumber);
            $("#hideflag").val("-");
            $("#chayeId").val(thisId);
            id=thisId;
        ...
        $('#plusOrReduceChayeKucunNumberModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#plusOrReduceChayeKucunNumber").val(0);
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#plusOrReduceChayeKucunNumberModal').modal('hide');
                $("#plusOrReduceChayeKucunNumberTitle").val("");
                $("#thisChayeKucunNumber").val("");
                $("#hideflag").val("");
                $("#chayeId").val("");
                getDataList();
                return;
        ...
        //模态框关闭的时候会执行这个里面的东西
        $('#plusOrReduceChayeKucunNumberModal').on('hide.bs.modal', function () {
            id=null;
            $("#plusOrReduceChayeKucunNumber").val(0);
            $("#plusOrReduceChayeKucunNumberTitle").val("");
            $("#thisChayeKucunNumber").val("");
        ...
        function submitPlusOrReduceChayeKucunNumber() {
            var id = $("#chayeId").val();//当前数据id
            var hideflag = $("#hideflag").val();//是加还是减
            var thisChayeKucunNumber = $("#thisChayeKucunNumber").val();//原先的数量
            var plusOrReduceChayeKucunNumber = $("#plusOrReduceChayeKucunNumber").val();//增加或者减少的数量
            var sumNumber=0;
            var msg;
            if(hideflag == "+"){
                sumNumber =parseInt(thisChayeKucunNumber) + parseInt(plusOrReduceChayeKucunNumber);
                msg = "增加库存成功";
            }else if(hideflag == "-"){
                sumNumber =parseInt(thisChayeKucunNumber) - parseInt(plusOrReduceChayeKucunNumber);
                msg = "减少库存成功";
            ...
            let data = {"id":id,"chayeKucunNumber":sumNumber};
            httpJson("chaye/update", "POST", data, (res) => {
                if(res.code == 0){
                    alert(msg);
                    getDataList();
