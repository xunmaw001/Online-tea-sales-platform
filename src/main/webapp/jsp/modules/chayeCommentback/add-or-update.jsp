            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑茶叶评价</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 chayeCommentbackTextDiv">
                                        <label>评价内容</label>
                                        <textarea style="width: 450px"  id="chayeCommentbackText" name="chayeCommentbackText" class="form-control"placeholder="评价内容"></textarea>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("chayeCommentback/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                chayeId: "required",
                yonghuId: "required",
                chayeCommentbackText: "required",
                replyText: "required",
            },
            messages: {
                chayeId: "茶叶不能为空",
                yonghuId: "用户不能为空",
                chayeCommentbackText: "评价内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchayeCommentback = window.sessionStorage.getItem("addchayeCommentback");
        if (addchayeCommentback != null && addchayeCommentback != "" && addchayeCommentback != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("chayeCommentback/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#chayeCommentbackText").val(ruleForm.chayeCommentbackText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#chayeCommentbackText').attr('readonly', 'readonly');
