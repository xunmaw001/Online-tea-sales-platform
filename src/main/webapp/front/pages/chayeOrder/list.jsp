...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的茶叶订单 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="modal-dialog" style="z-index: 3000">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <textarea id="chayeCommentbackContent" style="width: 500px;height: 300px"></textarea>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                    <button type="button" @click="submitChayeCommentback()" class="btn btn-theme">评价</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部茶叶订单</li>
                    <li v-for="item in chayeOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.chayeName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.chayePhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.chayeValue}}</td>
                            <td style="width: 60px;">{{item.chayeKucunNumber}}</td>
                            <td style="width: 60px;">{{item.chayePrice}}</td>
                            <td style="width: 100px;color: red">{{item.chayeOldMoney}}</td>
                            <td style="width: 100px;color: red">{{item.chayeNewMoney}}</td>
                            <td style="width: 60px;">{{item.chayeClicknum}}</td>
                            <td style="width: 60px;">{{item.zanNumber}}</td>
                            <td style="width: 60px;">{{item.caiNumber}}</td>
                            <td style="width: 80px;">{{item.shangxiaValue}}</td>
                            <td style="width: 150px;">{{item.chayeOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.buyNumber}}</td>
                            <td style="width: 150px;">{{item.chayeOrderTruePrice}}</td>
                            <td style="width: 150px;">{{item.chayeOrderCourierName}}</td>
                            <td style="width: 150px;">{{item.chayeOrderCourierNumber}}</td>
                            <td style="width: 80px;">{{item.chayeOrderValue}}</td>
                            <td style="width: 80px;">{{item.chayeOrderPaymentValue}}</td>
                            ...
                            <td style="width: 100px;">
                                ...
                                <button v-if="item.chayeOrderTypes==3" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 退款
                                </button>


                                <button v-if="item.chayeOrderTypes==4" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.chayeOrderTypes==5" @click="commentback(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius btn-theme">
...
<script>
    var vue = new Vue({
        ...
        // 当前表字典表数据
            chayeOrderTypesList : [],
            chayeOrderPaymentTypesList : [],
            chayeOrderTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            //列表查询
            ,pageList(chayeOrderTypes) {
                this.chayeOrderTypes = chayeOrderTypes;
                // 获取列表数据
                layui.http.request('chayeOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    chayeOrderTypes: (vue.chayeOrderTypes == 0)?null:vue.chayeOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('chayeOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    chayeOrderTypes: (this.chayeOrderTypes == 0)?null:this.chayeOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`chayeOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`chayeOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`chayeOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,submitChayeCommentback(){
                var chayeCommentbackContent = $("#chayeCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("chayeOrder/commentback?id="+id+"&commentbackText="+chayeCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function chayeOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chaye_order_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chayeOrderTypesList = res.data.list;
                }
            });
        }
        // 支付类型的查询
        function chayeOrderPaymentTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chaye_order_payment_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chayeOrderPaymentTypesList = res.data.list;
                }
            });
        }
        ...
        // 获取列表数据
        http.request('chayeOrder/page', 'get',{
            page: 1,
            limit: limit,
            chayeOrderTypes: (vue.chayeOrderTypes == 0)?null:vue.chayeOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('chayeOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            chayeOrderType: (vue.chayeOrderTypes == 0)?null:vue.chayeOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
