    ...
    <body>
                ...
                <table class="layui-table" lay-skin="nob" style="border: 3px dotted #EEEEEE;margin: 20px 0;">
                    ...
                    <tbody>
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td  style="width: 150px;">{{item.chayeNewMoney}} RMB</td>
                            <td style="width: 140px;">
                                {{item.buyNumber}}
                            </td>
                            <td style="width: 100px;">{{(item.chayeNewMoney*item.buyNumber).toFixed(2)}} RMB</td>
                        </tr>
                    </tbody>
                </table>

                <div class="btn-container">
                    <div v-if="this.chayeOrderPaymentTypes == 1" style="margin-right: 150px">
		...
		<script>
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.chayeNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						...
						let data={
                            ...
                            chayes:localStorage.getItem('chayes'),
                            yonghuId: localStorage.getItem('userid'),
                            chayeOrderPaymentTypes: vue.chayeOrderPaymentTypes,
						}
                        // 获取商品详情信息
                        layui.http.request(`chayeOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../chayeOrder/list.jsp';
							}
                       		 window.location.href='../chayeOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取茶叶购买的清单列表
				var chayes = localStorage.getItem('chayes');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(chayes);
