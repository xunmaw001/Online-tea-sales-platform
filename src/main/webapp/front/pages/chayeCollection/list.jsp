                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="chayeName" id="chayeName" placeholder="茶叶名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="chayeTypes" id="chayeTypes" lay-filter="chayeTypes">
                ...
                <div class="ng-inside-box ng-fault ng-container" aos="fade-up">
                    <div>
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 col-sm-3 col-xs-6 w3gallery-grids">
                            <i @click="deleteChayeCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                            <div class="imghvr-hinge-right figure" style="width: 100%" @click="jumpCheck('../chaye/detail.jsp?id='+item.chayeId , item.chayeDelete == null?'':item.chayeDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                <img :src="item.chayePhoto?item.chayePhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                <div class="agile-figcaption">
                                    <h4>{{item.chayeName}}</h4>
                                    <h4 v-if="item.chayeNewMoney">
                                        {{item.chayeNewMoney}} RMB
                                    </h4>
                                    <h4 v-else>
                                        {{item.chayeValue}}
                                    </h4>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            ,deleteChayeCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('chayeCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        chayeCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function chayeCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chaye_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.chayeCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function chayeTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chaye_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#chayeName').val() != null && jquery('#chayeName').val() != "" && jquery('#chayeName').val() != "null" ) {
                param['chayeName'] = jquery('#chayeName').val();
            }
                                         
            var chayeTypes = $("#chayeTypes").find("option:selected").val();
            if (chayeTypes != null && chayeTypes != "" && chayeTypes != "null") {
                            param['chayeTypes'] = chayeTypes;
                        }
                                                                                                                                                                                                                            
            // 当前表
                                                
            //收藏只能查类型为1的
            param['chayeCollectionTypes'] = 1;

            // 获取列表数据
            http.request('chayeCollection/page', 'get', param, function(res) {
